...
// --- Consistent JWT_SECRET (match authMiddleware.js) ---
...
// Connect to MongoDB
mongoose
  .connect(process.env.MONGO_URI || "mongodb://localhost:27017/onlineCompilerDB")
...
app.get("/dashboard", authMiddleware, async (req, res) => {
  try {
    ...
    let rating = acceptedCount * 10 - wrongCount * 2 - runtimeErrorCount * 1 - compilationErrorCount * 1 - tleCount * 3;
...
app.get("/profile/:userId", authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ user: { username: user.username, email }, submissions });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
